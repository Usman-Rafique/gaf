# Train direct synthesis network FDS
# Inputs: source image, flow (from AF++).

# The flow is provided by a trained AF++. By default, weights of a trained AF++ will be loaded from the 'checkpints' directory

# Before starting training, make sure that correct model name is set in config.py i.e. (cfg.model.name should be 'FDS') 
...
optim = torch.optim.Adam(fds.parameters(), lr=cfg.train.learning_rate, weight_decay=cfg.train.weight_decay)
...
for epoch in range(cfg.train.num_epochs):
    ...
    for i, data in enumerate(train_loader, 0):        
        ...
        loss_net1, discrim_loss = loss_function(image_gen, target, discrim, gan_loss, feature_loss, l1_loss)
